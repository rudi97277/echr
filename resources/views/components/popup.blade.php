<div id="info-popup"tabindex="-1" x-data="popupScript" x-init="setModal"
    ...
    <div class="relative p-4 w-full max-w-lg m-auto">
        <div class="relative p-4 rounded-lg shadow"
            :class="{
                'bg-danger text-white': error !== null,
                'bg-white text-dark border border-main': success !==
                    null
            }">
            <div class="mb-4 font-ligh">
                <p x-text="error || success" class="text-sm text-center text-balance"></p>
            </div>
            <div class="justify-between items-center pt-0 space-y-4 sm:flex sm:space-y-0">

                <div class="items-center w-full">
                    <button id="close-modal" type="button" class="w-full p-1 rounded-md bg-pale text-dark"
                        x-on:click="closeModal">Tutup</button>
                </div>
            </div>
        </div>
    </div>
