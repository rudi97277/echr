<div class="w-full h-full flex flex-col">
            ...
            <tbody>
                @foreach ($data ?? [] as $index => $item)
                    <tr class="bg-white border-b text-dark hover:bg-gray-50 ">
                        ...
                        @endforeach
                        <td>
                            <a class="bg-main rounded-md p-2 text-white"
                                href="{{ route('admin.employee-edit', AppHelper::obfuscate($item->id)) }}">Edit</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="text-sm py-3 border-t border-t-gray-200 flex pe-2">
        <p class="">Menampilkan data {{ $pagination['from'] ?? 0 }} sampai {{ $pagination['to'] ?? 0 }}</p>
