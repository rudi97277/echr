        ...
        <x-custom-table :$headers :source="$forms" :$pagination disableSearch="true">
            ...
            @scopedslot('action', $item)
                <div class="flex gap-1 ">
                    ...
                    <form action="{{ route('admin.form.delete', AppHelper::obfuscate($item->id)) }}" method="POST">
                        @csrf
                        @method('delete')
                        <div class="flex items-center rounded-md bg-danger text-white p-1 cursor-pointer">
                            <label for="submit">
                                <svg class="w-4 h-4 " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                                    height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <input id="submit" class="cursor-pointer" type="submit" value="Delete">
                        </div>
                    </form>
                </div>
            @endscopedslot
