            ...
            @scopedslot('action', $item)
                <div class="flex gap-1">
                    <a class="rounded-md bg-main text-white p-1 text-sm flex items-center"
                        href="{{ route('admin.master-jabatan.edit', AppHelper::obfuscate($item->id)) }}">
        ...
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <div class="relative bg-white rounded-lg shadow ">
                <form action="" method="POST" class="p-4">
                    @csrf
                    <h1 class="font-semibold text-lg mb-2">Tambah Master Jabatan</h1>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach ($salaries as $salary)
                                ...
                                <input type="text" disabled class="border-none bg-gray-200 text-sm rounded-md"
                                    value="{{ $salary->name }}">
                                <x-currency class="bg-pale border-none text-sm rounded-md w-full" name="amounts[]" />
                            @endforeach
